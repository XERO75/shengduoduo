...
import { createOrder } from "@/api/pintuan";
  ...
  methods: {
    ...
    onClickNext() {
      if(this.addressId){
        ...
        this.$router.push({path:'/pintuan/order',query:{productId: this.$route.query.productId}});
      }else{
        Toast('请选择配送地址');
      }
    },
    onClickAdd() {
      this.$router.push({path:'/pintuan/addAddress'});
    },
    onClickEdit(id) {
      this.$router.push({path:'/pintuan/editAddress',query:{id:id}});
    },
  },
  mounted(){
    handleLogin();
