...
import { getPintuanConfigure } from "@/api/pintuan";
...
export default {
  ...
  mounted(){
    handleLogin();
      ...
      // formdata.append('count',this.$store.state.pintuan.count);
      formdata.append('deliveryMethod',this.$store.state.pintuan.deliveryMethod);
      formdata.append('number',this.$store.state.pintuan.number);
      formdata.append('produtId',this.$store.state.pintuan.produtId);
      getPintuanConfigure(formdata).then(res=>{
